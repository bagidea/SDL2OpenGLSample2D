...
{
	glm::vec2 half(destRect.z / 2.0f, destRect.w / 2.0f);

	glm::vec2 t_l(-half.x, half.y);
	glm::vec2 b_l(-half.x, -half.y);
	glm::vec2 t_r(half.x, half.y);
	glm::vec2 b_r(half.x, -half.y);

	t_l = rotatePoint(t_l, angle) + half;
	b_l = rotatePoint(b_l, angle) + half;
	t_r = rotatePoint(t_r, angle) + half;
	b_r = rotatePoint(b_r, angle) + half;
	...
	newG->tl.color = color;
	newG->tl.setPosition(destRect.x + t_l.x, destRect.y + t_l.y);
	newG->tl.setUV(uvRect.x, uvRect.y + uvRect.w);

	newG->bl.color = color;
	newG->bl.setPosition(destRect.x + b_l.x, destRect.y + b_l.y);
	newG->bl.setUV(uvRect.x, uvRect.y);

	newG->br.color = color;
	newG->br.setPosition(destRect.x + b_r.x, destRect.y + b_r.y);
	newG->br.setUV(uvRect.x + uvRect.z, uvRect.y);

	newG->tr.color = color;
	newG->tr.setPosition(destRect.x + t_r.x, destRect.y + t_r.y);
	newG->tr.setUV(uvRect.x + uvRect.z, uvRect.y + uvRect.w);
...
glm::vec2 SpriteBatch::rotatePoint(glm::vec2 pos, float angle)
{
